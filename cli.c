...
static void flag(cli_t * cli, void * dest, enum flag_type type, cli_flag_options options) {
...
void cli_command(cli_t * cli, const char * name, cli_cmd_cb cb, const char * description, bool is_default, void * ctx) {
	...
	cli_cmd_t * cmd = calloc(1, sizeof(cli_cmd_t));
...
int cli_parse(cli_t * cli, int argc, const char ** argv) {
	...
	for ( i = 1; i < argc; i++ ) {
		...
		if (strcmp("-h", arg) == 0 || strcmp("--help", arg) == 0) return cli_usage(cli);
		...
		if (cmd == NULL) {
			cmd = (cli_cmd_t *) hash_get(cli->commands, arg_allocated);
			if (cmd) {
				free(arg_allocated);
				continue;
			}
	...
	if (cmd == NULL) return cli->has_commands ? cli_usage(cli) : 0;

	cli_cmd_cb cb = cmd->cb;
